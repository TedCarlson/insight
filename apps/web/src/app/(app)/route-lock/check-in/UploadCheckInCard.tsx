// apps/web/src/app/(app)/route-lock/check-in/UploadCheckInCard.tsx
...
export function UploadCheckInCard({
  uploadEnabled,
  ...
  async function onUpload() {
    ...
    try {
      ...
      const res = await fetch("/api/route-lock/check-in/upload", {
        method: "POST",
        body: fd,
      });
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <Card>
        <div className="space-y-3">
          <div>
            <div className="text-sm font-medium">Upload customer Check-In (XLSX)</div>
            <div className="text-sm text-[var(--to-ink-muted)]">
              Ingests job rows and refreshes day facts. Historical backfill is supported (past dates can insert/update).
            </div>
          </div>
              ...
              {result.ok ? (
                <div className="space-y-1">
                  ...
                  <div className="text-[var(--to-ink-muted)]">
                    Fulfillment Center ID: {result.fulfillment_center_id}
                    {result.min_cp_date || result.max_cp_date ? (
                      <>
                        {" "}
                        · CP Range: {result.min_cp_date ?? "—"} → {result.max_cp_date ?? "—"}
                      </>
                    ) : null}
                  </div>
                </div>
