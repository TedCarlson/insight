// apps/web/src/app/(app)/route-lock/check-in/page.tsx
...
import { UploadCheckInCard } from "./UploadCheckInCard";
import { ImportedRowsCardClient, type CheckInDayFactRow } from "@/features/route-lock/check-in/ImportedRowsCardClient";
import { RollupsCardClient } from "@/features/route-lock/check-in/RollupsCardClient";
...
export default async function CheckInRouteLockPage() {
  ...
  // Default window (historical, but lightweight): last 14 days ending today (inclusive)
  const from = addDaysISO(today, -13);
  const toExclusive = addDaysISO(today, 1);
  ...
  // Single pull: last 14 days day-facts
  const { data: rows } = await sb
    .from("check_in_day_fact")
    .select("shift_date, tech_id, actual_jobs, actual_units, actual_hours, first_start_time, last_cp_time, fiscal_end_date, fulfillment_center_id")
    .eq("pc_org_id", pc_org_id)
    .gte("shift_date", from)
    .lt("shift_date", toExclusive)
    .order("shift_date", { ascending: false })
    .order("tech_id", { ascending: true });

  const typedRows: CheckInDayFactRow[] = (rows ?? []) as any;
  ...
  return (
    <PageShell>
          ...
          left={
            <div className="min-w-0 flex items-center gap-2">
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold leading-5">Check-In</div>
                <div className="text-[11px] text-[var(--to-ink-muted)] leading-4">
                  Route Lock • Customer check-in ingestion + review
                </div>
              </div>
            </div>
          }
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <ImportedRowsCardClient rows={typedRows} today={today} pageSize={50} />
        <RollupsCardClient rows={typedRows} today={today} />
      </div>
    </PageShell>
  );
}