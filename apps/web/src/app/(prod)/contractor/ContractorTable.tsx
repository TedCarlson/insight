// apps/web/src/app/(prod)/contractor/ContractorTable.tsx

'use client'

import { useEffect, useMemo, useRef, useState } from 'react'
import { createContractor, deleteContractor, fetchContractors, updateContractor } from './contractor.api'
import type { ContractorInspectorMode, ContractorRow, CreateContractorInput, EditableField } from './contractor.types'
import ContractorInspector from './ContractorInspector'

const WRITE_DELAY_MS = 450

function getId(row: ContractorRow): string {
  const id = row.contractor_id ?? row.id
  return id ? String(id) : ''
}

function getName(row: ContractorRow): string {
  return String(row.contractor_name ?? row.name ?? '')
}

function getCode(row: ContractorRow): string {
  return String(row.contractor_code ?? row.code ?? '')
}

function getActive(row: ContractorRow): boolean {
...
export default function ContractorTable() {
  ...
  const [inspectorMode, setInspectorMode] = useState<ContractorInspectorMode>('create')
  ...
  const selectedContractor = useMemo(() => {
    ...
    return rows.find((r) => getId(r) === selectedContractorId) ?? null
  }, [rows, selectedContractorId])
  ...
  async function onCreate(payload: CreateContractorInput) {
    const created = await createContractor(payload)
    setRows((prev) => [created, ...prev])
  }

  async function onDelete(contractorId: string) {
    await deleteContractor(contractorId)
    setRows((prev) => prev.filter((r) => getId(r) !== contractorId))
  }

  function updateField(contractorId: string, field: EditableField, value: any) {
    // 1) optimistic update
    setRows((prev) =>
      prev.map((r) => {
        if (getId(r) !== contractorId) return r

        const next: ContractorRow = { ...(r as any) }

        if (field === 'name') {
          next.contractor_name = String(value ?? '')
          next.name = String(value ?? '')
        } else if (field === 'code') {
          next.contractor_code = value === '' ? null : String(value ?? '')
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setRows((prev) => prev.map((r) => (getId(r) === contractorId ? updated : r)))
      } catch (err: any) {
        console.error('Debounced contractor update error', err)
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <ContractorInspector
        open={inspectorOpen}
        mode={inspectorMode}
        contractor={inspectorMode === 'edit' ? selectedContractor : null}
