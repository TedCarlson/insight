...
function getCode(row: MsoRow | null | undefined): string {
  return String(row?.mso_code ?? row?.code ?? '')
}

function getActive(row: MsoRow | null | undefined): boolean {
...
export default function MsoInspector(props: {
  ...
  onDelete: (msoId: string) => Promise<void>
  onClose: () => void
}) {
  const { open, mode, mso, onChange, onCreate, onDelete, onClose } = props
  ...
  const nameValue = isCreate ? draftName : getName(mso)
  const codeValue = isCreate ? draftCode : getCode(mso)
  const activeValue = isCreate ? draftActive : getActive(mso)
  ...
  async function handleDelete() {
    if (!msoId) return
    const ok = window.confirm('Delete this MSO? This cannot be undone.')
    if (!ok) return

    try {
      setSaving(true)
      setSubmitError(null)
      await onDelete(msoId)
      onClose()
    } catch (err: any) {
      console.error('MSO delete error', err)
          ...
          <div className="col-span-12">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                else if (msoId) onChange(msoId, 'name', v)
              }}
          ...
          <div className="col-span-12 sm:col-span-6">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                else if (msoId) onChange(msoId, 'code', v)
              }}
            ...
            <div className="mt-2 flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  else if (msoId) onChange(msoId, 'active', v)
                }}
              />
