...
export async function GET(req: NextRequest) {
  ...
  if (!isOwner) {
    ...
    const perms: string[] = Array.isArray((passRes as any)?.data?.permissions) ? ((passRes as any).data.permissions as any) : [];
    const allowed = perms.includes("metrics_manage") || perms.includes("roster_manage");
