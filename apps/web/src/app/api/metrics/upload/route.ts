...
function buildUniqueKey(techId: string, orgId: string, fiscalEndDate: string, batchId: string) {
...
export async function POST(req: Request) {
  try {
    ...
    const pickedDate = form.get("picked_date") ? String(form.get("picked_date")) : null;
    const confirm = String(form.get("confirm") ?? "") === "1";
    const incomingBatchId = form.get("batch_id") ? String(form.get("batch_id")) : null;
    ...
    if (!sheetName) return json(400, { ok: false, error: "no sheets found" });

    const rows = sheetToRows(workbook, sheetName);
    if (!rows.length) return json(400, { ok: false, error: "empty sheet" });

    const title = rows?.[0]?.[0] !== null && rows?.[0]?.[0] !== undefined ? String(rows[0][0]) : null;
    const detected_generated_at = parseGeneratedAtFromTitle(title);

    const headerRow = rows?.[1];
    if (!Array.isArray(headerRow) || headerRow.length < 2) {
      return json(400, { ok: false, error: "missing header row (expected row 2)" });
    }
    ...
    if (detected_generated_at) {
      ...
      const detectedMonth = await resolveFiscalMonthByAnchorDate(supabase, detectedDateNY).catch(() => null);
    ...
    const dataRows = rows.slice(2).filter((r) => Array.isArray(r) && r.length > 0);
    ...
    if (row_count_total === 0) {
      return json(400, { ok: false, error: "no data rows found (TechId missing/blank)" });
    }
    ...
    if (batchGetErr) return json(500, { ok: false, error: batchGetErr.message });
    if (!batchRow) return json(400, { ok: false, error: "batch not found" });
    if (String(batchRow.pc_org_id) !== String(pc_org_id)) return json(403, { ok: false, error: "forbidden" });
    if (String(batchRow.status) !== "staged") {
      return json(400, { ok: false, error: "batch is not staged", detail: { status: batchRow.status } });
    }
    ...
    const toInsert = normalizedTechIds.map(({ tech_id, raw }) => {
      const unique_row_key = buildUniqueKey(tech_id, pc_org_id, fiscalEndForKey, batch_id);
      return {
        batch_id,
        pc_org_id,
    ...
    if (toInsert.length) {
      const { error: insErr } = await supabase.from("metrics_raw_row").insert(toInsert);
      if (insErr) {
        await supabase
          .from("metrics_raw_batch")
