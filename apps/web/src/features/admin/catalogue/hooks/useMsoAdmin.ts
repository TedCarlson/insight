...
export type MsoAdminRow = {
  mso_id: string;
  mso_name: string | null;
  mso_lob: string | null;
};

type MsoAdminResponse = {
  rows: MsoAdminRow[];
  page: { pageIndex: number; pageSize: number; totalRows?: number };
};

export function useMsoAdmin(opts?: { pageSize?: number }) {
  const [q, setQ] = useState("");
  const [pageIndex, setPageIndex] = useState(0);
  const [pageSize, setPageSize] = useState(opts?.pageSize ?? 25);

  const [data, setData] = useState<MsoAdminResponse | null>(null);
  ...
  const fetchNow = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/catalogue/mso?${sp.toString()}`);
      const json = (await res.json()) as { rows?: MsoAdminRow[]; page?: any; error?: string };
