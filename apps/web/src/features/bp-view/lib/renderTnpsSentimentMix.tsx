...
export function renderTnpsSentimentMix(args: {
  totalSurveys: number;
  ...
  const passive = Math.max(
    0,
    args.totalSurveys - args.totalPromoters - args.totalDetractors
  );
  ...
  return (
    <div key="sentiment-mix" className="rounded-2xl border bg-muted/10 p-4">
      ...
      <div className="mt-3 grid grid-cols-4 gap-2">
        <MixCard label="Surveys" value={args.totalSurveys || "—"} />
        <MixCard label="Pro" value={args.totalPromoters || 0} tone="success" />
        <MixCard label="Pass" value={passive} tone="warning" />
        <MixCard label="Det" value={args.totalDetractors || 0} tone="danger" />
      </div>
    </div>
  );
}