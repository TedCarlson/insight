...
import type { LeadershipRollupRow } from "../lib/buildLeadershipRollupRows";
...
export default function CompanyManagerLeadershipTable(props: {
  rows: LeadershipRollupRow[];
  activeLeader: string | null;
  onSelectLeader: (leader: string | null) => void;
}) {
  const { rows, activeLeader, onSelectLeader } = props;
  const [showMix, setShowMix] = useState(false);

  if (!rows.length) {
    return (
      <div className="rounded-2xl border bg-muted/[0.04] p-4 text-sm text-muted-foreground">
        No leadership rows in scope.
      </div>
    );
  }

  const metricColumns = rows[0]?.metric_order ?? [];
  const gridTemplate = `220px repeat(${metricColumns.length}, minmax(84px, 1fr)) ${
    showMix ? "repeat(4, minmax(84px, 1fr)) " : ""
  }72px`;
  const minWidth = showMix ? "1420px" : "1080px";
  ...
  return (
    <div className="rounded-2xl border bg-muted/[0.04] p-4">
      ...
      <div className="overflow-auto rounded-2xl border">
        ...
        >
          <DesktopHeaderCell>Leader</DesktopHeaderCell>
        ...
        {rows.map((row) => {
          const active = activeLeader === row.leader_key;
          const titleDisplay = row.leader_title ? ` • ${row.leader_title}` : "";

          return (
            <div
              key={row.leader_key}
              onClick={() => onSelectLeader(active ? null : row.leader_key)}
              className={[
            ...
            >
              <DesktopCell strong>
                {row.leader_name}
                <span className="text-muted-foreground font-normal">{titleDisplay}</span>
              </DesktopCell>
