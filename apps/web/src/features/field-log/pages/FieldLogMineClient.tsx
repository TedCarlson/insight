...
function getHref(row: MineRow) {
  if (row.status === "draft") return `/field-log/draft/${row.report_id}`;
  if (row.status === "tech_followup_required" && row.edit_unlocked) {
    return `/field-log/draft/${row.report_id}`;
  }
  return `/field-log/${row.report_id}`;
}

export function FieldLogMineClient() {
          ...
          {rows.map((row) => {
            ...
            return (
              <Link
                key={row.report_id}
                href={href}
                className="block rounded-2xl border bg-card p-4 transition hover:bg-muted/40"
              >
                ...
                <div className="mt-3 text-xs font-medium text-muted-foreground">
                  {editable ? "Opens editable draft" : "Opens submission detail"}
                </div>
              </Link>
            );
          })}
