// path: apps/web/src/features/metrics/components/ImportedMetricsRowsCardClient.tsx
...
export function ImportedMetricsRowsCardClient({
  ...
  const filtered = useMemo(() => {
    ...
    if (fiscalEnd) out = out.filter((r) => r.fiscal_end_date === fiscalEnd);

    if (qNorm) {
      out = out.filter(
        (r) => includesCI(r.tech_id, qNorm) || includesCI(r.unique_row_key, qNorm)
      );
    }
    ...
    <Card>
      <div className="space-y-3">
        ...
        <div className="max-h-[680px] overflow-auto rounded-md border border-[color:var(--to-border)]">
          <table className="w-full text-sm">
            ...
            <tbody>
              {total === 0 ? (
                <tr>
                  <td className="py-6 pl-3 text-[var(--to-ink-muted)]" colSpan={3}>
        ...
        <div className="text-xs text-[var(--to-ink-muted)]">
          Safe patch: raw bucket evidence only. Single bulk TPR upload enters here first.
        </div>
      </div>
    </Card>
  );
