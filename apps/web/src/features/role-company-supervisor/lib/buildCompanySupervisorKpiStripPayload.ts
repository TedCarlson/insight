// path: apps/web/src/features/role-company-supervisor/lib/buildCompanySupervisorKpiStripPayload.ts
...
function resolveBandFromRubric(args: {
  ...
  const normalizedRows = args.rubricRows
    .map((row) => ({
      band_key: normalizeBandKey(String((row as any).band_key ?? null)),
  ...
  for (const row of normalizedRows) {
    const meetsMin =
      row.min_value == null || args.value >= row.min_value;
    const meetsMax =
      row.max_value == null || args.value <= row.max_value;
