// path: apps/web/src/features/role-company-supervisor/lib/getCompanySupervisorSurfacePayload.server.ts
...
import { resolveCompanySupervisorScope } from "./resolveCompanySupervisorScope";

type CompanySupervisorProfileKey = "NSR" | "SMART";

function normalizeProfileKey(
  value: string | null | undefined
): CompanySupervisorProfileKey {
  ...
  const scopedTechIds = Array.from(
    new Set(
      resolvedScope.scoped_assignments
        .map((row: any) => String(row.tech_id ?? "").trim())
        .filter(Boolean)
    )
  );
