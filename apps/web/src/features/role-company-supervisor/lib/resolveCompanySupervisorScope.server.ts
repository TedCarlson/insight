import { bootstrapProfileServer } from "@/lib/auth/bootstrapProfile.server";
import { requireSelectedPcOrgServer } from "@/lib/auth/requireSelectedPcOrg.server";
import { supabaseAdmin } from "@/shared/data/supabase/admin";
import { fetchActiveRosterForOrg } from "@/shared/lib/activeRoster";

type TeamClass = "ITG" | "BP";

export type CompanySupervisorScopeAssignmentRow = {
  assignment_id: string | null;
  person_id: string | null;
  pc_org_id: string | null;
  tech_id: string | null;
  position_title: string | null;
  active: boolean | null;
  start_date?: string | null;
  end_date?: string | null;
  team_class?: TeamClass;
  contractor_name?: string | null;
};

export type CompanySupervisorScopePersonRow = {
  person_id: string;
  full_name: string | null;
  role: string | null;
  co_ref_id: string | null;
};

export type CompanySupervisorScopeRole = "Company Supervisor";

export type CompanySupervisorBpSupervisorGroup = {
  assignment_id: string;
  person_id: string | null;
  label: string;
  tech_ids: string[];
};

export type CompanySupervisorScopeResult = {
  selected_pc_org_id: string;
  supervisor_person_id: string;
  role_label: CompanySupervisorScopeRole;
  rep_full_name: string | null;
  company_label: string | null;
  scoped_assignments: CompanySupervisorScopeAssignmentRow[];
  people_by_id: Map<string, CompanySupervisorScopePersonRow>;
  org_labels_by_id: Map<string, string>;
  bp_supervisor_groups: CompanySupervisorBpSupervisorGroup[];
};

type AssignmentLeadershipRow = {
  child_assignment_id: string | null;
  parent_assignment_id: string | null;
  active: boolean | null;
};

function isoToday() {
  return new Date().toISOString().slice(0, 10);
}

function isActiveWindow(
  row: {
    active?: boolean | null;
    start_date?: string | null;
    end_date?: string | null;
  },
  today: string
) {
  const activeOk = row.active === true || row.active == null;
  const startOk = !row.start_date || String(row.start_date) <= today;
  const endOk = !row.end_date || String(row.end_date) >= today;
  return activeOk && endOk;
}

function toMaybeString(value: unknown) {
  const out = String(value ?? "").trim();
  return out || null;
}

async function loadOrgLabels(
  admin: ReturnType<typeof supabaseAdmin>,
  pcOrgIds: string[]
) {
  const out = new Map<string, string>();
  if (!pcOrgIds.length) return out;

  const { data } = await admin
    .from("pc_org")
    .select("pc_org_id,pc_org_name")
    .in("pc_org_id", pcOrgIds);

  for (const row of data ?? []) {
    if (row?.pc_org_id) {
      out.set(
        String(row.pc_org_id),
        row?.pc_org_name ? String(row.pc_org_name) : String(row.pc_org_id)
      );
    }
  }

  for (const id of pcOrgIds) {
    if (!out.has(id)) out.set(id, id);
  }

  return out;
}

function uniqueByTech(rows: CompanySupervisorScopeAssignmentRow[]) {
  const out = new Map<string, CompanySupervisorScopeAssignmentRow>();

  for (const row of rows) {
    const techId = String(row.tech_id ?? "").trim();
    if (!techId) continue;
    if (!out.has(techId)) out.set(techId, row);
  }

  return [...out.values()];
}

function isItgAssignment(args: {
  assignment: CompanySupervisorScopeAssignmentRow;
  person: CompanySupervisorScopePersonRow | undefined;
}) {
  const role = String(args.person?.role ?? "").toLowerCase();
  const positionTitle = String(args.assignment.position_title ?? "").toLowerCase();

  return (
    role === "hires" ||
    role === "employee" ||
    role === "employees" ||
    role === "itg" ||
    positionTitle.includes("itg")
  );
}

function isBpLeadershipAssignment(assignment: CompanySupervisorScopeAssignmentRow) {
  const positionTitle = String(assignment.position_title ?? "").toLowerCase();

  return (
    positionTitle.includes("bp owner") ||
    positionTitle.includes("bp lead") ||
    positionTitle.includes("bp supervisor")
  );
}

function buildChildrenByParent(rows: AssignmentLeadershipRow[]) {
  const out = new Map<string, string[]>();

  for (const row of rows) {
    const parentId = String(row.parent_assignment_id ?? "").trim();
    const childId = String(row.child_assignment_id ?? "").trim();
    if (!parentId || !childId) continue;
    if (row.active === false) continue;

    const arr = out.get(parentId) ?? [];
    arr.push(childId);
    out.set(parentId, arr);
  }

  return out;
}

function collectDescendantAssignmentIds(args: {
  seedIds: string[];
  childrenByParent: Map<string, string[]>;
}) {
  const out = new Set<string>();
  const queue = [...args.seedIds];

  while (queue.length) {
    const current = queue.shift();
    if (!current) continue;
    if (out.has(current)) continue;

    out.add(current);

    const children = args.childrenByParent.get(current) ?? [];
    for (const childId of children) {
      if (!out.has(childId)) queue.push(childId);
    }
  }

  return out;
}

export async function resolveCompanySupervisorScope(): Promise<CompanySupervisorScopeResult> {
  const [boot, scope] = await Promise.all([
    bootstrapProfileServer(),
    requireSelectedPcOrgServer(),
  ]);

  if (!boot.ok || !boot.person_id) {
    throw new Error("No linked person profile");
  }

  if (!scope.ok) {
    throw new Error("No org selected");
  }

  const admin = supabaseAdmin();
  const today = isoToday();
  const selected_pc_org_id = scope.selected_pc_org_id;

  const [meRes, activeRoster, myAssignmentsRes, leadershipRes] = await Promise.all([
    admin
      .from("person")
      .select("person_id,full_name,role,co_ref_id")
      .eq("person_id", boot.person_id)
      .maybeSingle(),
    fetchActiveRosterForOrg(admin as any, selected_pc_org_id),
    admin
      .from("assignment_admin_v")
      .select(
        "assignment_id,person_id,pc_org_id,tech_id,start_date,end_date,position_title,active"
      )
      .eq("pc_org_id", selected_pc_org_id)
      .eq("person_id", boot.person_id),
    admin
      .from("assignment_leadership_admin_v")
      .select("child_assignment_id,parent_assignment_id,active")
      .eq("active", true),
  ]);

  const me = (meRes.data ?? null) as CompanySupervisorScopePersonRow | null;

  if (!me) {
    throw new Error("Unable to resolve current person");
  }

  if (myAssignmentsRes.error) {
    throw new Error(`Unable to load supervisor assignments: ${myAssignmentsRes.error.message}`);
  }

  if (leadershipRes.error) {
    throw new Error(`Unable to load assignment leadership: ${leadershipRes.error.message}`);
  }

  const activePersonIds = [...activeRoster.personIds];

  const [assignmentRes, peopleRes] = await Promise.all([
    activePersonIds.length
      ? admin
          .from("assignment_admin_v")
          .select(
            "assignment_id,person_id,pc_org_id,tech_id,start_date,end_date,position_title,active"
          )
          .eq("pc_org_id", selected_pc_org_id)
          .in("person_id", activePersonIds)
      : Promise.resolve({ data: [], error: null } as any),
    activePersonIds.length
      ? admin
          .from("person")
          .select("person_id,full_name,role,co_ref_id")
          .in("person_id", Array.from(new Set([...activePersonIds, boot.person_id])))
      : Promise.resolve({ data: [], error: null } as any),
  ]);

  if (assignmentRes.error) {
    throw new Error(`Unable to load org assignments: ${assignmentRes.error.message}`);
  }

  if (peopleRes.error) {
    throw new Error(`Unable to load people for scope: ${peopleRes.error.message}`);
  }

  const allOrgAssignments = ((assignmentRes.data ?? []) as CompanySupervisorScopeAssignmentRow[])
    .filter((a) => isActiveWindow(a, today));

  const myAssignments = ((myAssignmentsRes.data ?? []) as CompanySupervisorScopeAssignmentRow[])
    .filter((a) => isActiveWindow(a, today));

  const childrenByParent = buildChildrenByParent(
    (leadershipRes.data ?? []) as AssignmentLeadershipRow[]
  );

  const myAssignmentIds = myAssignments
    .map((a) => String(a.assignment_id ?? "").trim())
    .filter(Boolean);

  const descendantAssignmentIds = collectDescendantAssignmentIds({
    seedIds: myAssignmentIds,
    childrenByParent,
  });

  const scopedAssignmentsSource = uniqueByTech(
    allOrgAssignments.filter((a) =>
      descendantAssignmentIds.has(String(a.assignment_id ?? "").trim())
    )
  );

  const people_by_id = new Map<string, CompanySupervisorScopePersonRow>();
  for (const row of (peopleRes.data ?? []) as CompanySupervisorScopePersonRow[]) {
    const personId = String(row.person_id ?? "").trim();
    if (!personId) continue;
    people_by_id.set(personId, row);
  }

  const normalizedAssignments = uniqueByTech(
    scopedAssignmentsSource.map((assignment) => {
      const person = people_by_id.get(String(assignment.person_id ?? "").trim());
      const team_class: TeamClass = isItgAssignment({ assignment, person }) ? "ITG" : "BP";

      return {
        ...assignment,
        team_class,
        contractor_name: null,
      };
    })
  );

  const org_labels_by_id = await loadOrgLabels(admin, [selected_pc_org_id]);

  const bpLeaderAssignments = normalizedAssignments.filter(
    (a) => a.team_class === "BP" && isBpLeadershipAssignment(a)
  );

  const bp_supervisor_groups: CompanySupervisorBpSupervisorGroup[] =
    bpLeaderAssignments.map((leader) => {
      const seedId = String(leader.assignment_id ?? "").trim();
      const bpDescendants = collectDescendantAssignmentIds({
        seedIds: seedId ? [seedId] : [],
        childrenByParent,
      });

      const tech_ids = normalizedAssignments
        .filter((a) => {
          const assignmentId = String(a.assignment_id ?? "").trim();
          return (
            a.team_class === "BP" &&
            Boolean(assignmentId) &&
            bpDescendants.has(assignmentId) &&
            Boolean(String(a.tech_id ?? "").trim())
          );
        })
        .map((a) => String(a.tech_id ?? "").trim())
        .filter(Boolean);

      const label =
        people_by_id.get(String(leader.person_id ?? "").trim())?.full_name?.trim() ||
        String(leader.position_title ?? "").trim() ||
        "BP Supervisor";

      return {
        assignment_id: seedId,
        person_id: leader.person_id ?? null,
        label,
        tech_ids,
      };
    });

  return {
    selected_pc_org_id,
    supervisor_person_id: boot.person_id,
    role_label: "Company Supervisor",
    rep_full_name: me.full_name ?? null,
    company_label: org_labels_by_id.get(selected_pc_org_id) ?? null,
    scoped_assignments: normalizedAssignments,
    people_by_id,
    org_labels_by_id,
    bp_supervisor_groups,
  };
}