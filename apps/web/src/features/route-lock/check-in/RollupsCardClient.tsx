...
type DayAgg = { day: string; jobs: number; units: number; hours: number };
...
export function RollupsCardClient({ rows, today }: { rows: CheckInDayFactRow[]; today: string }) {
  const roll = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const dayAgg = byDay.get(day) ?? { day, jobs: 0, units: 0, hours: 0 };
      dayAgg.jobs += j;
      dayAgg.units += u;
      dayAgg.hours += h;
      byDay.set(day, dayAgg);

      const techAgg = byTech.get(r.tech_id) ?? { tech_id: r.tech_id, jobs: 0, units: 0, hours: 0, days: new Set<string>() };
      techAgg.jobs += j;
      techAgg.units += u;
      techAgg.hours += h;
  ...
  return (
    <Card>
      <div className="min-w-0">
        ...
        <div className="text-[11px] text-[var(--to-ink-muted)] leading-4">Window summary + top techs</div>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        ...
        <div className="rounded-md border border-[var(--to-border)] p-3">
          <div className="text-[11px] text-[var(--to-ink-muted)]">Rows</div>
          <div className="text-base font-semibold tabular-nums">{fmtInt(roll.rowCount)}</div>
        </div>
        <div className="rounded-md border border-[var(--to-border)] p-3">
      ...
      {roll.todayAgg ? (
        <div className="mt-4 rounded-md border border-[var(--to-border)] bg-black/5 p-3">
          ...
          <div className="mt-1 text-sm tabular-nums">
            {fmtInt(roll.todayAgg.jobs)} jobs · {fmtNum(roll.todayAgg.units, 1)} units · {fmtNum(roll.todayAgg.hours, 1)} hrs
          </div>
        </div>
      ) : null}
      ...
      <div className="mt-4">
          ...
          {roll.days.length === 0 ? (
            ...
            roll.days.slice(-14).map((d) => (
              <div key={d.day} className="flex items-center justify-between rounded-md border border-[var(--to-border)] px-3 py-2">
                ...
                <div className="text-[11px] text-[var(--to-ink-muted)] tabular-nums">
                  {fmtInt(d.jobs)} jobs · {fmtNum(d.units, 1)} units · {fmtNum(d.hours, 1)} hrs
                </div>
              </div>
            ))
          )}
        ...
        {roll.days.length > 14 ? (
          <div className="mt-2 text-[11px] text-[var(--to-ink-muted)]">Showing last 14 days of daily totals.</div>
        ) : null}
      </div>
    </Card>
  );
