...
function comparisonPillClass(
  state: MetricsExecutiveKpiItem["comparison_state"]
) {
  if (state === "better") {
...
function PrimaryKpiCard({ item }: { item: MetricsExecutiveKpiItem }) {
  return (
...
function SecondaryKpiCard({ item }: { item: MetricsExecutiveKpiItem }) {
  return (
