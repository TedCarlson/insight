// path: apps/web/src/shared/surfaces/MetricsParityCard.tsx
...
export type MetricsParityGroupType = "COMPANY" | "CONTRACTOR";
...
export type MetricsParityRow = {
  group_type: MetricsParityGroupType;
  label: string;
  rank_display?: string | null;
  hc: number;
...
type Props = {
  title?: string;
  rows: MetricsParityRow[];
  detailRows?: MetricsParityRow[];
};
...
function displayGroupType(groupType: MetricsParityGroupType) {
    ...
    <button
      ...
      className={[
        ...
        props.disabled
          ...
          : "hover:bg-[color-mix(in_oklab,var(--to-primary)_14%,white)]",
      ].join(" ")}
    >
      {props.label}
    </button>
...
export default function MetricsParityCard({
  title = "Parity",
  rows,
  detailRows,
}: Props) {
  ...
  const toggleLabel = mode === "summary" ? "Show Detail" : "Show Summary";
