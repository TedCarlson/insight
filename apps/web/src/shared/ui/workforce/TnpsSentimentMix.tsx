...
type Props = {
  ...
  totalPromoters: number | null | undefined;
  totalDetractors: number | null | undefined;
  title?: string;
};

export default function TnpsSentimentMix(props: Props) {
  const surveys =
    typeof props.totalSurveys === "number" && Number.isFinite(props.totalSurveys)
      ? props.totalSurveys
      : 0;

  const promoters =
    typeof props.totalPromoters === "number" &&
    Number.isFinite(props.totalPromoters)
      ? props.totalPromoters
      : 0;

  const detractors =
    typeof props.totalDetractors === "number" &&
    Number.isFinite(props.totalDetractors)
      ? props.totalDetractors
      : 0;
  ...
  return (
    <div className="rounded-2xl border bg-muted/10 p-4">
      ...
      <div className="mt-3 grid grid-cols-4 gap-2">
        <MixCard label="Surveys" value={surveys || "—"} />
        <MixCard label="Pro" value={promoters || 0} tone="success" />
        <MixCard label="Pass" value={passive} tone="warning" />
        <MixCard label="Det" value={detractors || 0} tone="danger" />
      </div>
    </div>
  );
}